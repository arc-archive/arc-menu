...
import { UrlIndexer } from '@advanced-rest-client/arc-models';
import { commitValue } from '../src/SearchMenuElement.js';
...
describe('SavedMenuElement', () => {
  ...
  async function modelFixture() {
    ...
    `);
    return /** @type SearchMenuElement */ node.querySelector('search-menu');
  }
  ...
  describe('Results rendering', () => {
    ...
    before(() => {
      ...
      items.push(saved);
      const hItem = generator.generateHistoryObject();
  ...
  describe('querying for data', () => {
    ...
    let historyRequests;
    before(async () => {
      const projects = await generator.insertProjectsData({ projectsSize: 2 });
      const insert = await generator.insertSavedRequestData({
        forceProject: true,
        autoRequestId: true,
        projects,
      });
      ...
      historyRequests = await generator.insertHistoryRequestData();
