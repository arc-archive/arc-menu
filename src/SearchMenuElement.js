 ...
 */
export class SearchMenuElement extends ListMixin(LitElement) {
  static get styles() {
   ...
   */
  [formTemplate]() {
    const { compatibility, q, querying } = this;
    return html`
  ...
  [historyItemTemplate](searchItem, index) {
    const { compatibility, draggableEnabled, hasTwoLines } = this;
    ...
    >
      <anypoint-item-body ?twoline="${hasTwoLines}" ?compatibility="${compatibility}">
